...
function waitelem(self, selector, done) {
  ...
  var newDone = function (err) {
    if (err) {
      return done(
        new Error(
          `.wait() for ${selector} timed out after ${self.options.waitTimeout
          }msec`
        )
      )
    }
...
exports.evaluate = function (fn /**, arg1, arg2..., done**/) {
  ...
  var timeoutTimer = setTimeout(function () {
      ...
      new Error(
        `Evaluation timed out after ${self.options.executionTimeout
...
exports.cookies.clearAll = function (done) {
  ...
  console.warn('clear cookies is disabled due to incompatibility')
  done();
}
